...
class AuthControllerTest {
  ...
  void register_ValidRequest_ShouldReturnToken() throws Exception {
    ...
    when(jwtUtil.generateToken(anyString(), anyString(), anyString(), any()))
  ...
  void getCurrentUser_ValidToken_ShouldReturnUser() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(get("/api/auth/me").header("Authorization", token))
        .andExpect(status().isOk())
  ...
  void refreshToken_ValidToken_ShouldReturnNewToken() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(post("/api/auth/refresh").header("Authorization", token))
        .andExpect(status().isOk())
