...
@ContextConfiguration(classes = {MessageController.class, TestSecurityConfig.class})
class MessageControllerTest {
  ...
  void getMessagesByRoom_UserIsMember_ShouldReturnMessages() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/messages/room/test-room-id").header("Authorization", "Bearer valid-token"))
  ...
  void getMessagesByRoom_InvalidToken_ShouldReturnUnauthorized() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/messages/room/test-room-id").header("Authorization", "Bearer invalid-token"))
  ...
  void getRecentMessages_UserIsMember_ShouldReturnMessages() throws Exception {
    ...
    when(messageService.findRecentMessages(eq("test-room-id"), any(LocalDateTime.class)))
        .thenReturn(messages);

    // When & Then
    mockMvc
        .perform(
            get("/api/messages/room/test-room-id/recent")
  ...
  void searchMessages_UserIsMember_ShouldReturnMatchingMessages() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/messages/room/test-room-id/search")
                .param("q", "test")
                .header("Authorization", "Bearer valid-token"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content[0].id").value("test-message-id"));
  ...
  void getMessagesByDateRange_UserIsMember_ShouldReturnMessages() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/messages/room/test-room-id/date-range")
  ...
  void getMessagesByUser_SameUser_ShouldReturnMessages() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/messages/user/test-user-id").header("Authorization", "Bearer valid-token"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content[0].id").value("test-message-id"));
  ...
  void getMessagesByUser_DifferentUser_ShouldReturnForbidden() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/messages/user/other-user-id").header("Authorization", "Bearer valid-token"))
        .andExpect(status().isForbidden());
  ...
  void getMessageById_UserHasAccess_ShouldReturnMessage() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(get("/api/messages/test-message-id").header("Authorization", "Bearer valid-token"))
        .andExpect(status().isOk())
  ...
  void getMessageById_MessageNotFound_ShouldReturnNotFound() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/messages/nonexistent-message").header("Authorization", "Bearer valid-token"))
  ...
  void getMessageStats_UserIsMember_ShouldReturnStats() throws Exception {
    ...
    when(messageService.getRecentMessageCount(eq("test-room-id"), any(LocalDateTime.class)))
        .thenReturn(10L);

    // When & Then
    mockMvc
        .perform(
            get("/api/messages/room/test-room-id/stats")
                .header("Authorization", "Bearer valid-token"))
        .andExpect(status().isOk())
  ...
  void deleteMessage_AuthorCanDelete_ShouldDeleteMessage() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/api/messages/test-message-id").header("Authorization", "Bearer valid-token"))
  ...
  void deleteMessage_MessageNotFound_ShouldReturnNotFound() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/api/messages/nonexistent-message")
                .header("Authorization", "Bearer valid-token"))
        .andExpect(status().isNotFound());
