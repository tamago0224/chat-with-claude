...
public interface MessageRepository extends JpaRepository<Message, String> {
    ...
    @Query("SELECT m FROM Message m WHERE m.room.id = :roomId AND m.content ILIKE %:searchTerm% ORDER BY m.createdAt DESC")
    ...
    long countByRoomIdAndCreatedAtAfter(@Param("roomId") String roomId, @Param("since") LocalDateTime since);
    
    @Query("SELECT m FROM Message m WHERE m.room.id = :roomId AND m.createdAt BETWEEN :startDate AND :endDate ORDER BY m.createdAt DESC")
