...
public class MessageController {
    ...
    public ResponseEntity<Page<Message>> searchMessages(@PathVariable String roomId,
    ...
    public ResponseEntity<Page<Message>> getMessagesByDateRange(@PathVariable String roomId,
        ...
        Page<Message> messages = messageService.findMessagesByDateRange(roomId, startDate, endDate, pageable);
        return ResponseEntity.ok(messages);
    }

    @GetMapping("/user/{userId}")
    public ResponseEntity<Page<Message>> getMessagesByUser(@PathVariable String userId,
                                                         Pageable pageable,
                                                         @RequestHeader("Authorization") String token) {
    ...
    public ResponseEntity<Map<String, Object>> getMessageStats(@PathVariable String roomId,
                                                             @RequestHeader("Authorization") String token) {
        String userId = getUserIdFromToken(token);
        if (userId == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        ...
        long recentMessages = messageService.getRecentMessageCount(roomId, LocalDateTime.now().minusDays(1));
