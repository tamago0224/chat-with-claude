...
public class ChatRoomController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ChatRoom> getRoomById(@PathVariable String id, @RequestHeader("Authorization") String token) {
        String userId = getUserIdFromToken(token);
        if (userId == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ChatRoom> updateRoom(@PathVariable String id, 
    ...
    @PostMapping("/{id}/join")
    public ResponseEntity<Map<String, String>> joinRoom(@PathVariable String id, 
                                                      @RequestHeader("Authorization") String token) {
        String userId = getUserIdFromToken(token);
        if (userId == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
    ...
    public ResponseEntity<Map<String, String>> leaveRoom(@PathVariable String id, 
                                                       @RequestHeader("Authorization") String token) {
        String userId = getUserIdFromToken(token);
        if (userId == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
    ...
    public ResponseEntity<List<RoomMember>> getRoomMembers(@PathVariable String id, 
                                                         @RequestHeader("Authorization") String token) {
        String userId = getUserIdFromToken(token);
        if (userId == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
    ...
    @GetMapping("/search")
    public ResponseEntity<Page<ChatRoom>> searchPublicRooms(@RequestParam String q, Pageable pageable) {
